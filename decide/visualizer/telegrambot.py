...
def send_telegram_report_voting(voting):
        ...
        status = "Not started voting"
        if voting_start is not None:     
            if voting_end is None:
                a = str(voting_start).split(" ")[0].split("-")
                start_date=a[0] + " - " + a[1] + " - " + a[2]
                status = "Started voting. Date: " + start_date
            else:
                a = str(voting_end).split(" ")[0].split("-")
                end_date=a[0] + " del " + a[1] + " de " + a[2]
                status = "Finished voting. Date: " + end_date

        options = voting_q.options.all()
        post = voting.postproc

        text = "Not started voting\n\n"
        text += "Voting: " + str(voting_id) + " - " + voting_name +"\n"
        text += "Status: " + status + "\n"
        text += "Description: " + str(voting_desc) + "\n\n"
        if voting_tally is None:
            text += "Question: " + str(voting_q) + "\n"
            for option in options:
                text += "Option: " + str(option.number) + " - " + option.option+ "\n"
            if voting_end is not None:
                text += "\nWaiting tally"
        else:
            text += "Question: " + str(voting_q) + "\n"
            for option in post:
                text += "Option " + str(option.get('number')) + " - " + option.get('option') + " - " + str(option.get('votes')) + " votes \n"
            text += "\nResults: \n"
            for option in post:
...
def send_telegram_report_binary(v):
    ...
    if voting_start is not None:     
        if voting_end is None:
            ...
            a = str(voting_end).split(" ")[0].split("-")
            end_date=a[2] + " - " + a[1] + " - " + a[0]
            status = "Finished voting. Date:  " + end_date
    text = "Preference voting Report\n\n"
    text += "Voting: " + str(voting_id) + " - " + voting_title +"\n"
    text += "Status: " + status + "\n"
    text += "Description: " + str(voting_desc) + "\n\n"
    text += "Yes: " +str(VotacionBinaria.Numero_De_Trues(v)) + "\n"
    text += "No: " +str(VotacionBinaria.Numero_De_Falses(v)) + "\n"
    bot_token = '1415070510:AAE49OJPu4viYNo5Tfov4vzkoIyeNf_JBr4'
    bot = telegram.Bot(bot_token)
    id = -1001318632551
    return bot.send_message(id,text)
...
def send_telegram_report_mult(v):
    ...
    if voting_start is not None:     
        if voting_end is None:
            ...
            a = str(voting_end).split(" ")[0].split("-")
            end_date=a[0] + " - " + a[1] + " - " + a[2]
            status = "Finish voting. Date: " + end_date
    text = "Multiple voting Report\n\n"
    text += "Voting: " + str(voting_id) + " - " + voting_title +"\n"
    text += "Status: " + status + "\n"
    text += "Description: " + str(voting_desc) + "\n\n"
    for question in voting_questions:
        ...
        if PreguntaMultiple.Numero_De_Opciones(question) == 0 :
            text += "There are not any options. \n"
        else:
